...
| Prod.mk α β => Prod.mk β α

def pair := ("Hello", 5)
#eval prod_comm pair --expect (5, "Hello")
...
| Prod.mk β α => Prod.mk α β

#eval prod_com_reverse (prod_comm pair) --expect ("Hello, 5")
...
| Prod.mk α (Prod.mk β γ) => Prod.mk (Prod.mk α β) γ

def triple := ("Hello", true, 5)
#eval triple
#eval prod_assoc triple --expect (("Hello, true"), 5)
/-!
...
| Prod.mk (Prod.mk α β) γ => Prod.mk α (Prod.mk β γ)

#eval prod_assoc_reverse (prod_assoc triple) --expect ("Hello", true, 5)
/-!
...
a *γ* value. 
 ...
 | Prod.mk α (Sum.inl β) => Sum.inl (Prod.mk α β)
 | Prod.mk α (Sum.inr γ) => Sum.inr (Prod.mk α γ)
...
def prod_dist_sum_rev {α β γ : Type} : (α × β) ⊕ (α × γ) → α × (β ⊕ γ)
| Sum.inl (Prod.mk α β) => Prod.mk α (Sum.inl β)
| Sum.inr (Prod.mk α γ) => Prod.mk α (Sum.inr γ)
...
rain ⊕ sprinkler → (rain → wet) → (sprinkler → wet) → wet
| Sum.inl rain, rain2wet, _ => rain2wet rain
...
α ⊕ β → (α → γ) → (β → γ) → γ
|Sum.inl α, α2γ, _ => α2γ α
|Sum.inr β, _, β2γ => β2γ β
